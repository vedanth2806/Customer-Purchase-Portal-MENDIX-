...
{
    static final String APPROOTURL = Core.getConfiguration().getApplicationRootUrl().endsWith("/") ?
            Core.getConfiguration().getApplicationRootUrl() : Core.getConfiguration().getApplicationRootUrl().concat("/");

    static final String OAUTHCALLBACKENDPOINT = APPROOTURL.concat(Constants.getOAuthCallbackPath().endsWith("/") ?
    ...
    static void createAndShowError(OAuthErrors err, Session sessionAssociation, HttpServletResponse servletResponse) throws IOException, CoreException, OAuthException {
        ...
        servletResponse.sendRedirect(OAuthHelper.APPROOTURL + "p/oautherror_et/" + err.getMendixObject().getId().toLong());
    }
    ...
    static void fetchOrRefreshToken(EmailSettings account, IContext context) throws OAuthException, IOException, CoreException {
        ...
        if(instant.getEpochSecond() < Instant.now().getEpochSecond()) {
            ...
            {
                case AZURE:
                    newToken = new AzureOAuthHandler(providerDetails).refreshExpiredToken(token, context);
                    break;
